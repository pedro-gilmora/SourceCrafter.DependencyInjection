        ...
        var servicesContainers = context.SyntaxProvider
                ...
                .Select(static (result, c) =>
                {
                    ...
                    try
                    {
                        ...
                        var requiresProviderInterfaces = false;
                        ...
                        if (interceptorsCount > 0 || requiresProviderInterfaces)
                            files.Add(("Utils.g", BuildUtilsFileContent(interceptorsCount > 0, requiresProviderInterfaces)));
...
#pragma warning restore CS9113";
    ...
    /// forma explicita, una vez por tipo registrado, y la API generica resuelve con una
    /// prueba de tipo (<c>this is IProvider&lt;TOut&gt;</c>) en lugar de comparar
    /// <c>typeof(TOut).FullName</c> contra una lista de literales.
    /// </summary>
    private const string ProviderInterfacesContent = @"
namespace SourceCrafter.DependencyInjection
{
    internal interface IProvider<T> where T : notnull
    {
        T GetService();
    }

    internal interface IAsyncProvider<T> where T : notnull
    {
        global::System.Threading.Tasks.Task<T> GetServiceAsync();
    }

    internal interface IMultipleProvider<T> where T : notnull
    {
        T[] GetServices();
    }

    internal interface IMultipleAsyncProvider<T> where T : notnull
    {
        global::System.Threading.Tasks.Task<T[]> GetServicesAsync();
    }
}";

    private static string BuildUtilsFileContent(bool interceptors, bool providerInterfaces)
    {
        StringBuilder code = new("// <auto-generated/>\n#nullable enable\n");

        if (interceptors) code.Append(InterceptsLocationContent).Append('\n');

        if (providerInterfaces) code.Append(ProviderInterfacesContent).Append('\n');
